...
class App extends Component {
  ...
  validarRegistro = () => {
    ...
    if (nombre.length === 0) {
      ...
      axios.post('https://tweelock-apis.azurewebsites.net/registrarUsuario', usuario)
        .then((response) => {
  ...
  iniciarSesion = () => {
    ...
    if (username.length === 0) {
      ...
      axios.get(`https://tweelock-apis.azurewebsites.net/verificarUsuario/${username}&${password}`)
        .then((response) => {
          console.log(response.data)
          if (response.data === 'Usuario no registrado en el sistema.') {
            this.setState({ modalSesion: true, modalHeader: 'No existe usuario', modalMessage: response.data })
          } else {
            this.setState({ modalSesion: true, modalHeader: 'Inicio de sesión exitoso', modalMessage: response.data })
          }
      ...
      <div>
        <form>
          ...
          <div className={styles.contenedorSubmit}>
            <a className={styles.btnSubmit} onClick={this.iniciarSesion}>Iniciar</a>
          </div>
        </form>
      </div>
    )
